...
{
    [PluginActionId("com.linariii.battery")]
    public class Battery : ActionBase
    {
        private const int DataFetchCooldownSec = 300;

        public Battery(SDConnection connection, InitialPayload payload) : base(connection, payload)
        {
            if (payload.Settings == null || payload.Settings.Count == 0)
            {
                Settings = BatteryPluginSettings.CreateDefaultSettings();
            }
            else
            {
#if DEBUG
...
#endif
                Settings = payload.Settings.ToObject<BatteryPluginSettings>();
            }
        ...
        protected BatteryPluginSettings Settings
        {
            get
            {
                var settings = BaseSettings as BatteryPluginSettings;
                if (settings == null)
